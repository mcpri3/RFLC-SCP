...
# protected areas and 3. calculates multi-scale network metrics based on the probability of connectivity (PC) metric and betweenness
############################################################################################################################################################
...
#########################################################################################################################
# The output files are located in /outputs/EdgeLists and /outputs/Networks folder 
...
for (i in 1:nrow(lst.param)) { #loop on each parameter combination, preferably run on a distant cluster each combination in parallel
  ...
  if (sum(class(corrid) == 'try-error') == 0) { #if an EC has been estimated
    ...
    if (nrow(inter)>0) { #if EC intersects PAs
      ...
      if (nrow(edgelst.final)>0) { #if interPAs links exists without filtering on distance  
        ...
        if (nrow(edgelst.final)>0) { #if interPAs links exists after removing too long euclidean links
          ...
          if (nrow(edgelst.updated)>0){ #if interPAs links exists after removing too long geographic links 
...
##################################################################################################################################
####### 3. Calculate multi-scale network metrics based on the probability of connectivity (PC) metric and betweenness  ###########
##################################################################################################################################
# The output files are located in /outputs/Indicators folder 
...
all.ep$site.area.km2 <- sf::st_area(all.ep)/1000000 #calculate PA area
ep.dist <- readRDS(here::here('data/raw-data/ProtectedAreas/Distance_btw_StrictProtections')) #matrix of Euclidean distance between PAs
connector = F #PC_connector can be calculated but can take a while, default = FALSE 
...
for (i in 1:nrow(lst.param)) { #loop on each parameter combination, preferably run on a distant cluster each combination in parallel
  ...
  if (sum(class(corrid) == 'try-error') == 0) { #if an EC has been estimated
    ...
    # Read network and edge list file 
    net <- try(readRDS(here::here(paste0('outputs/Networks/StrictPANetwork_',lst.param[i, 2], '_GroupID_', lst.param[i, 3], '_TransfoCoef_', lst.param[i, 4],
                                         '_SuitThreshold_', lst.param[i, 5], '_DispDist_', lst.param[i, 6], 'km_NormFlowThreshold_', lst.param[i, 7]))))
    if (sum(class(net) == 'try-error') == 0) { #if a network exists
      
      edgelst.final <- read.csv(here::here(paste0('outputs/EdgeLists/StrictPAEdgeList_',lst.param[i, 2], '_GroupID_', lst.param[i, 3], '_TransfoCoef_', lst.param[i, 4],
                                                  '_SuitThreshold_', lst.param[i, 5], '_DispDist_', lst.param[i, 6], 'km_NormFlowThreshold_', lst.param[i, 7], '.csv')))
      ...
      foreach::foreach(p=names(V(net))) %do% { #run on each patch p 
        ...
        if (sum(idx) != 0) {
          ...
          if (connector == T) { # calculate the connector metric
      ...
      btw <- data.frame(nodeID = names(V(net)), btw = igraph::betweenness(net, directed = F, normalized = T))
      ...
      btw.full <- sf::st_drop_geometry(all.ep)
    ...
    } else { #if no network but EC exists
      
      # Only PCintra might not be equal to 0 
      PC_i <- PC_intra_i
      PC_i$PC_flux_i <- 0
      ...
      indic.con$Betwness <- data.frame(sf::st_drop_geometry(all.ep), btw = 0)
      indic.con$Betwness$site.area.km2 <- as.numeric(indic.con$Betwness$site.area.km2)
      
    }
  }
  
  if (sum(class(corrid) == 'try-error') != 0) { #No EC has been estimated
