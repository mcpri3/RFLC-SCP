...
PRE_FATE.speciesClustering_step1 = function(mat.species.DIST, opt.no_clust_max = 15)
{
  ...
  if(!.testParam_notInClass(mat.species.DIST, "list"))
  {
    if (length(mat.species.DIST) > 0)
    {
      ...
      {
        if (!.testParam_notInClass(mat.species.DIST[[i]], c("dist", "niolap", "matrix")))
        {
  ...
  clust.evaluation = foreach(group = combi$GROUP, no.clusters = combi$no.clusters) %do%
    {
      ...
      stats = fpc::cluster.stats(mat.species.DIST[[group]], c1, c2)
      ...
      mdunn = clValid::dunn(mat.species.DIST[[group]], c1)
  ...
  clust.evaluation = reshape2::melt(clust.evaluation, id.vars = c("GROUP","no.clusters"))
